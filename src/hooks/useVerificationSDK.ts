...
import { logger, inspectSDKState, createTimer, debugBreakpoint } from '@/utils/debug';
...
export function useVerificationSDK() {
  ...
  const vecuIDVRef = useRef<InstanceType<typeof import('vecu-idv-web-sdk').VecuIDV> | null>(null);
  ...
  const initializeSDKVerification = useCallback(async (docvToken: string) => {
    ...
    try {
      ...
      // Load the Socure provider (this triggers auto-connect event forwarding)
      const providerTimer = createTimer('Provider Loading');
      logger.sdk('Loading Socure provider...');
      // @ts-expect-error - Accessing private property for test mode
      const provider = await sdkInstance.providerLoader.load('socure');
      providerTimer.end();
      
      logger.sdk('Provider loaded successfully - auto-connect event forwarding should be active');
      
      // Initialize the verification UI with DocV token
      logger.sdk('Initializing verification UI with container and DocV token');
      ...
      logger.sdk('Verification UI initialized successfully with hardcoded test setup');
