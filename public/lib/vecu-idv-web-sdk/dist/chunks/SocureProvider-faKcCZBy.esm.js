        ...
        try {
            ...
            const socureConfig = {
                ...
                onSuccess: (response) => {
                    ...
                    console.log('[SOCURE DEBUG] onSuccess - checking if DOCUMENTS_UPLOADED needs manual progress trigger');
                    if ('status' in response && response.status === 'DOCUMENTS_UPLOADED') {
                        console.log('[SOCURE DEBUG] 🔧 Manually triggering handleProgressEvent for DOCUMENTS_UPLOADED');
                        this.handleProgressEvent(response, options.sessionId);
                        return;
                    }
                    console.log('[SOCURE DEBUG] Processing non-DOCUMENTS_UPLOADED completion in onSuccess');
                    if (!('status' in response) || response.status !== 'DOCUMENTS_UPLOADED') {
...
//# sourceMappingURL=SocureProvider-faKcCZBy.esm.js.map
