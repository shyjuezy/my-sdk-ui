import { EventEmitter, Logger, VecuError, TimeoutError, ProviderError } from '../index.esm.js';
...
class BaseProvider {
    ...
    async loadScript(url, globalName) {
        return new Promise((resolve, reject) => {
            ...
            script.onerror = (error) => {
                ...
                reject(new VecuError('PROVIDER_SDK_LOAD_FAILED', `Failed to load ${this.name} SDK from ${url}`));
            };
    ...
    handleProviderError(error, context) {
        const vecuError = error instanceof VecuError
            ? error
            : new VecuError('PROVIDER_ERROR', `${this.name} provider error in ${context}: ${error instanceof Error ? error.message : String(error)}`, this.name);
    ...
    validateInitOptions(options) {
        if (!options.sessionId) {
            throw new VecuError('INVALID_OPTIONS', 'Session ID is required');
        }
        if (!options.token) {
            throw new VecuError('INVALID_OPTIONS', 'Token is required');
        }
        if (!options.container || !(options.container instanceof HTMLElement)) {
            throw new VecuError('INVALID_OPTIONS', 'Valid HTML container element is required');
        }
        if (!['modal', 'embedded'].includes(options.mode)) {
            throw new VecuError('INVALID_OPTIONS', 'Mode must be either "modal" or "embedded"');
        }
    }
}
...
class SocureSDKLoader {
    ...
    async loadSDK() {
        ...
        const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => {
                reject(new TimeoutError(`Socure SDK load timeout after ${timeoutMs}ms`, timeoutMs));
            }, timeoutMs);
        });
...
class SocureProvider extends BaseProvider {
    ...
    async initializeVerification(options) {
        try {
            ...
            if (!this.socureDocVSDK) {
                throw new ProviderError(this.name, 'Socure DocV SDK not loaded');
            }
            ...
            if (!sdkKey) {
                throw new ProviderError(this.name, 'Socure SDK key is required in provider config');
            }
            console.log('[SOCURE DEBUG] SDK Key:', sdkKey.substring(0, 10) + '...');
            const docvTransactionToken = options.token;
            if (!docvTransactionToken) {
                throw new ProviderError(this.name, 'docvTransactionToken is required');
            }
            ...
            const socureConfig = {
                ...
                onSuccess: (response) => {
                    ...
                    if ('status' in response && response.status === 'DOCUMENTS_UPLOADED') {
                        console.log('[SOCURE DEBUG] Processing DOCUMENTS_UPLOADED - generating completion message');
                        const completionMessage = this.getCompletionMessage(response);
                        console.log('[SOCURE DEBUG] Generated message:', completionMessage);
                        const eventData = {
                            sessionId: options.sessionId,
                            message: completionMessage,
                            result: {
                                status: 'completed',
                                docvTransactionToken: response.docvTransactionToken,
                                deviceSessionToken: response.deviceSessionToken,
                                customerUserId: response.customerUserId
                            }
                        };
                        console.log('[SOCURE DEBUG] Emitting verification:completed event with data:', eventData);
                        this.emitProviderEvent('verification:completed', eventData);
                    }
                    else {
                        console.log('[SOCURE DEBUG] Using old/fallback completion structure');
    ...
    processWebhookData(data) {
        try {
            ...
            throw new ProviderError(this.name, 'Failed to process webhook data');
        }
    }
    mapEvent(event) {
...
export { SocureProvider };
//# sourceMappingURL=SocureProvider-CJ3pIHZg.esm.js.map
